...
export function WorkspacePage({ kind, onCreate }: { kind: PageKind; onCreate: () => void }) {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      const normalized: Job[] = rows.map((row: unknown) => normalize(row));
  ...
  const rows = useMemo(() => jobs.filter(job => kind === "queue" ? ["queued", "running", "completed", "failed", "cancelled"].includes(job.status) : kind === "library" ? job.status === "completed" && job.outputPath : true).filter(job => !query || `${job.title}${engines[job.engine].name}${job.status}`.toLowerCase().includes(query.toLowerCase())), [jobs, kind, query]);
  const action = async (id: string, name: "cancel" | "retry") => { try { const response = await fetch(apiUrl(`/api/jobs/${id}/${name}`), { method: "POST" }); if (!response.ok) throw new Error(await response.text()); setConfirm(null); await refresh(); } catch (reason) { setError(reason instanceof Error ? reason.message : "操作失败"); } };
  ...
  return <div className="data-page">
    ...
    {audioUrl && <div className="library-player"><div><Play size={18} /><span><strong>正在播放</strong><small>{audioTitle}</small></span></div><audio src={audioUrl} controls autoPlay /><button onClick={() => setAudioUrl("")} aria-label="关闭播放器"><X size={18} /></button></div>}
    ...
    <section className="data-surface">
      {loading ? <div className="page-state"><LoaderCircle className="spin" size={30} /><strong>正在读取本地任务</strong><span>这通常只需要几秒钟。</span></div> : error ? <div className="page-state error"><AlertCircle size={30} /><strong>加载失败</strong><span>{error}</span><button className="secondary-button" onClick={refresh}><RefreshCw size={17} />重试</button></div> : rows.length === 0 ? <div className="page-state"><config.icon size={34} /><strong>{query ? "没有匹配结果" : kind === "library" ? "还没有完成的音频" : kind === "queue" ? "当前没有生成任务" : "暂无历史记录"}</strong><span>{query ? "尝试更换搜索词。" : "从创作台创建第一个语音任务。"}</span>{!query && <button className="primary-button" onClick={onCreate}>前往创作台</button>}</div> : <div className="data-table"><div className="data-table-head"><span>任务</span><span>引擎</span><span>状态 / 进度</span><span>更新时间</span><span>操作</span></div>{rows.map(job => <article className="data-row" key={job.id}><div className="data-job"><span className="data-job-icon"><FileAudio size={18} /></span><span><strong>{job.title}</strong><small>{job.segmentsTotal ? `${job.segmentsDone}/${job.segmentsTotal} 个分段` : job.outputPath || "等待处理"}</small></span></div><span className="engine-pill">{engines[job.engine].name}</span><div className="data-progress"><div><span className={`status-dot ${job.status}`} />{statusLabel(job.status)}<strong>{job.progress}%</strong></div><div className="progress-track"><i style={{ width: `${job.progress}%` }} /></div>{job.error && <small>{job.error}</small>}</div><time>{formatDate(job.updatedAt || job.createdAt)}</time><div className="row-actions">{job.status === "completed" && job.outputPath && <button onClick={() => playOutput(job)} title="试听音频"><Play size={17} /></button>}{kind === "library" && job.outputPath && <button onClick={() => exportOutput(job)} title="导出音频副本"><Download size={17} /></button>}{job.outputPath && <button onClick={() => showOutput(job)} title="打开文件位置"><FolderOpen size={17} /></button>}{["queued", "running"].includes(job.status) && <button onClick={() => setConfirm({ id: job.id, title: job.title })} title="取消任务"><Square size={16} /></button>}{["failed", "cancelled"].includes(job.status) && <button onClick={() => action(job.id, "retry")} title="重试"><RotateCcw size={17} /></button>}{job.outputPath && <button onClick={() => navigator.clipboard.writeText(job.outputPath!)} title="复制路径"><Copy size={17} /></button>}</div></article>)}</div>}
    </section>
    {confirm && <div className="confirm-overlay" role="presentation"><div className="confirm-dialog" role="alertdialog" aria-modal="true" aria-labelledby="confirm-title"><span className="confirm-icon"><Pause size={22} /></span><h2 id="confirm-title">取消这个生成任务？</h2><p>“{confirm.title}”已完成的分段会保留，之后可以从断点重试。</p><div><button className="secondary-button" onClick={() => setConfirm(null)}>继续生成</button><button className="danger-button" onClick={() => action(confirm.id, "cancel")}>确认取消</button></div><button className="confirm-close" onClick={() => setConfirm(null)} aria-label="关闭"><X size={18} /></button></div></div>}
  </div>;
}
