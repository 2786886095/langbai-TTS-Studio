...
export function WorkspacePage({ kind, onCreate }: { kind: PageKind; onCreate: () => void }) {
  ...
  const rows = useMemo(() => jobs.filter(job => kind === "queue" ? ["queued", "running", "failed", "cancelled"].includes(job.status) : kind === "library" ? job.status === "completed" && job.outputPath : true).filter(job => !query || `${job.title}${engines[job.engine].name}${job.status}`.toLowerCase().includes(query.toLowerCase())), [jobs, kind, query]);
