...
function VoiceFields({ engine, values, onChange }: { engine: EngineId; values: Record<string, unknown>; onChange: (key: string, value: unknown) => void }) {
  ...
  return <>
    ...
    <PathField label="辅助参考音频（可选）" value={values.aux_ref_audio_paths} kind="audio" onChange={value => onChange("aux_ref_audio_paths", value)} help="需要补充音色或语气特征时使用；会跟随角色声音独立保存。" />
